...
__global__ void matmul_kernel(int M, int N, int K, half* d_A, half* d_B, half* d_C) {
    ...
    __shared__ half shm_B[2 * BLK_N * BLK_K];
    ...
    for (int k = 0; k < K / MMA_K - 1; k++) {
        pipe_load(d_A, d_B, shm_A, shm_B, M, N, K, k+1);
        __pipeline_commit();
        __pipeline_wait_prior(1);
        pipe_calc(shm_A, shm_B, reg_A, reg_B, reg_C, k);
    }
