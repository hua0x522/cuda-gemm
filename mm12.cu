...
__device__ void load_shm_A(half* shm_A, half* A, int M, int K, int ko) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        __pipeline_memcpy_async(
            &shm_A[row * 64 + shm_col],
            &A[(blockIdx.x * 128 + row) * K + ko * 64 + col],
            16
        );
...
__device__ void load_reg_A(uint32_t* reg_A, half* shm_A, int ki) {
    for (int m = 0; m < 8; m++) {
        ...
        int shm_col = col ^ ((row & 7) << 3);
        uint32_t shm_A_lane_addr = __cvta_generic_to_shared(shm_A + row * 64 + shm_col);
...
__device__ void load_reg_B(uint32_t* reg_B, half* shm_B, int ki) {
    int lane_id = threadIdx.x;
        ...
        int shm_col = col ^ ((row & 7) << 3);
        uint32_t shm_B_lane_addr = __cvta_generic_to_shared(shm_B + row * 64 + shm_col);
...
__device__ void store_C(uint32_t* reg_C, half* C, int M, int N) {
    ...
    for (int m = 0; m < 8; m++) {
        for (int n = 0; n < 2; n++) {
            ...
            C[(row + 8) * N + col] = __float2half(*(float*)&reg_C[m * 8 + n * 4 + 2]);
            C[(row + 8) * N + col + 1] = __float2half(*(float*)&reg_C[m * 8 + n * 4 + 3]);
        }
    }
}
...
__global__ void matmul_kernel(int M, int N, int K, half* d_A, half* d_B, half* d_C) {
    ...
    __pipeline_wait_prior(0);
    pipe_calc(shm_A, shm_B, reg_A, reg_B, reg_C, K / 64 - 1);
...
void matmul(int M, int N, int K, half* h_A, half* h_B, half* h_C) {
    ...
    matmul_kernel<<<dim3(cdiv(M, 128), cdiv(N, 64)), dim3(32, 4)>>>(M, N, K, d_A, d_B, d_C);
